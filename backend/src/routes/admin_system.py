"""Admin routes: host/system summary, throughput, about, GPUs, node-exporter host metrics,
capabilities and Docker image cache status."""
...
from ..schemas.admin import (
    SystemSummary, ThroughputSummary, GpuMetrics,
    HostSummary, HostTrends, Capabilities,
)
...
async def system_about(settings = Depends(get_settings), _: dict = Depends(require_admin)):
    """Facts the UI quotes in guides and the About page (version, pinned images, paths, ports)."""
    import os
    return {
        "version": settings.CORTEX_VERSION or "dev",
        "vllm_image": settings.VLLM_IMAGE,
        "llamacpp_image": settings.LLAMACPP_IMAGE,
        "host_ip": os.environ.get("HOST_IP") or None,
        "gateway_port": 8084,
        "frontend_port": int(os.environ.get("FRONTEND_PORT") or 3001),
        "models_dir": settings.CORTEX_MODELS_DIR_HOST or settings.CORTEX_MODELS_DIR,
        "hf_cache_dir": settings.HF_CACHE_DIR_HOST or settings.HF_CACHE_DIR,
        "export_dir": settings.CORTEX_EXPORT_DIR_HOST or settings.CORTEX_EXPORT_DIR,
        "offline_mode": bool(settings.OFFLINE_MODE),
        "dev_allow_all_keys": bool(settings.GATEWAY_DEV_ALLOW_ALL_KEYS),
